...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    DesignerNotification *notification = self.dataSource[indexPath.row];
    NSString *type = notification.message_type;
    
    if ([NotificationBusiness contains:type inFilter:[NotificationBusiness designerSystemAnnouncementFilter]]) {
        ...
        return cell;
    } else if ([NotificationBusiness contains:type inFilter:[NotificationBusiness designerRequirmentNotificationFilter]]) {
        ...
        return cell;
    } else if ([NotificationBusiness contains:type inFilter:[NotificationBusiness designerWorksiteNotificationFilter]]) {
...
- (void)refreshDatasource {
    switch (self.currentNotificationType) {
        case NotificationTypeAll:
            self.filter = [NotificationBusiness designerAllNotificationsFilter];
            self.dataSource = self.dataManager.allNotifications;
            break;
        case NotificationTypeAnnouncement:
            self.filter = [NotificationBusiness designerSystemAnnouncementFilter];
            self.dataSource = self.dataManager.systemAnnouncements;
            break;
        case NotificationTypeRequirement:
            self.filter = [NotificationBusiness designerRequirmentNotificationFilter];
            self.dataSource = self.dataManager.requirmentNotifications;
            break;
        case NotificationTypeWorksite:
            self.filter = [NotificationBusiness designerWorksiteNotificationFilter];
...
- (void)refresh {
    ...
    SearchDesignerNotification *request = [SearchDesignerNotification requestWithTypes:self.filter];
    request.from = @(0);
    request.limit = @(20);
    
    [API searchDesignerNotification:request success:^{

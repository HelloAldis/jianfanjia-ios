...
+ (void)showUserAvatarSelector {
    [self showPhotoSelector:YES isMultiSelection:NO withMaxSelection:1];
}

+ (void)showDecorationNodeImageSelector:(NSInteger)count {
    [self showPhotoSelector:NO isMultiSelection:YES withMaxSelection:count];
}

+ (void)showPhotoSelector:(BOOL)allowsEditing isMultiSelection:(BOOL)allowsMultiSection withMaxSelection:(NSInteger)maxCount {

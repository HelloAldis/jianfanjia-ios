...
typedef NS_ENUM(NSInteger, PhotoUtilCropperStyle) {
    PhotoUtilCropperStyleOriginal,
    PhotoUtilCropperStyleSquare,
};
...
+ (void)showUploadProductImageSelector:(nonnull UIViewController *)controller inView:(UIView *)sourceView max:(NSInteger)count withBlock:(FinishUploadBlock)block {
    [self showImageBrowser:controller allowsEditing:NO isMultiSelection:YES withMaxSelection:count style:PhotoUtilCropperStyleOriginal withBlock:block];
}
...
+ (void)showAwardImageSelector:(nonnull UIViewController *)controller inView:(UIView *)sourceView max:(NSInteger)count withBlock:(FinishUploadBlock)block {
    [self showPhotoSelector:controller inView:sourceView allowsEditing:YES isMultiSelection:YES withMaxSelection:count style:PhotoUtilCropperStyleOriginal withBlock:block];
}

+ (void)showPhotoSelector:(nonnull UIViewController *)controller inView:(UIView *)sourceView allowsEditing:(BOOL)allowsEditing isMultiSelection:(BOOL)allowsMultiSection withMaxSelection:(NSInteger)maxCount style:(PhotoUtilCropperStyle)style withBlock:(FinishUploadBlock)block {
    ...
    UIAlertAction *photo = [UIAlertAction actionWithTitle:@"从相册中选取" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self showImageBrowser:controller allowsEditing:allowsEditing isMultiSelection:allowsMultiSection withMaxSelection:maxCount style:style withBlock:block];
    }];
...
+ (void)showImageBrowser:(UIViewController *)controller allowsEditing:(BOOL)allowsEditing isMultiSelection:(BOOL)allowsMultiSection withMaxSelection:(NSInteger)maxCount style:(PhotoUtilCropperStyle)style withBlock:(FinishUploadBlock)block  {
